...
from data_provider.data_factory_zhongqi import data_provider
    ...
    def test(self, setting, test=0):
        ...
        with torch.no_grad():
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(test_loader):
                ...
                f_dim = -1 if self.args.features == 'MS' else 0
                outputs = outputs[:, -self.args.day_len:, :]
                batch_y = batch_y[:, -self.args.day_len:, :].to(self.device)
        ...
        indices = np.arange(0, preds.shape[0] -1, self.args.day_len)
        ...
        acc_10d = np.mean(1 -np.sqrt(np.mean((preds -trues) **2, axis=1))[:30] /self.args.Cap)
        print('10day(>0.7):', self.args.pred_len /self.args.day_len, acc_10d)
