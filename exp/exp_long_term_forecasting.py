...
class Exp_Long_Term_Forecast(Exp_Basic):
    ...
    def test(self, setting, test=0):
        ...
        indices = np.arange(preds.shape[0] -1, 0, -self.args.day_len)
        ...
        riqian = kaohes /sum(sum(trues[:30]))
        print('day-ahead:(<0.07)',riqian)

        for i in range(30):
            plt.figure(figsize=(12, 6))
            plt.plot(trues[i], label='True Values')
        ...
        # pred_h = preds.reshape(-1, 24, 4).mean(axis=2)
        # true_h = trues.reshape(-1, 24, 4).mean(axis=2)
        # rmse_h = np.sqrt(np.mean((pred_h - true_h) ** 2, axis=1))
        # Acc_h = 1 - rmse_h / self.args.Cap
        # count = np.sum(Acc_h >= 0.80)
        # ra = count / pred_h.shape[0]
