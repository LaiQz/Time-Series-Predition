...
from data_provider.data_factory_rinei import data_provider
...
class Exp_Long_Term_Forecast(Exp_Basic):
    ...
    def _select_criterion(self):
        criterion =  nn.MSELoss()#nn.L1Loss() #LogCoshLoss()#
        return criterion
    ...
    def test(self, setting, test=0):
        ...
        with torch.no_grad():
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(test_loader):
                ...
                batch_x_mark = batch_x_mark.float().to(self.device)
                batch_y_mark = batch_y_mark.float().to(self.device)[:, -self.args.pred_len:]
        ...
        preds = np.concatenate(preds, axis=0).clip(min=0)[:,-4:]
        trues = np.concatenate(trues, axis=0)[:,-4:]
        print('test shape:', preds.shape)

        # cost = time.time() - time_now

        indices = np.arange(0, preds.shape[0] -1, 4)
        slices_p = [preds[i] for i in indices]
        preds = np.flip(np.array(slices_p), axis=0)[:720]
        slices_t = [trues[i] for i in indices]
        trues = np.flip(np.array(slices_t), axis=0)[:720]
        print('test shape:', preds.shape)
        ...
        for m in range(720):
            ...
            for n in range(4):
                ...
                kaohe = max(np.abs(pred - true) -p_delta, 0) *alpha
                kaohes.append(kaohe)
        kaohes = sum(np.array(kaohes))
        rinei = kaohes / sum(sum(trues))
        print('Intra-day:(<0.05)', rinei)
        ...
        # np.savetxt(f'D:\github code\\baka\Time-Series-Library-main\pred\er_{self.args.aaa}.txt', np.array([mape, acc]))
