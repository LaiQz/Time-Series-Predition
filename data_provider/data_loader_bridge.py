...
class Dataset_ETT_hour(Dataset):
    def __init__(self, args, root_path, flag='train', size=None,
                 ...
                 target='OT', scale=0, timeenc=0, freq='h', seasonal_patterns=None):
...
class Dataset_Custom(Dataset):
    def __init__(self, args, root_path, flag='train', size=None,
                 features='S', data_path='ETTh1.csv',
                 target='Target', scale=0, timeenc=1, freq='h', seasonal_patterns=None):
        # size [seq_len, label_len, pred_len]
        self.args = args
        # info
        if size == None:
    ...
    def __read_data__(self):
        ...
        if os.path.exists(local_fp):
            df_raw = pd.read_csv(local_fp)#.iloc[-96 *60:]
            df_train = pd.read_csv('.\data\\bridge_hr.csv')
            # if self.args.aaa >=3:
            #     df_raw.drop(df_raw.iloc[:, 16-self.args.aaa:14].columns, axis=1, inplace=True)
        else:
            ds = load_dataset(HUGGINGFACE_REPO, name=cfg_name)
        ...
        if self.set_type == 0:
            self.data_x = df_train_0.values
            self.data_y = df_train_0.values
